                                                ...
                                                LDataManager* const l_data_manager)
{
    ...
    // Get periodic shift to determine which directions are periodic.
        ...
        for (LNodeSetData::CellIterator cit(patch_box); cit; cit++)
        {
            ...
            if (mstr_node_set)
            {
                     ...
                     scit; scit++)
                {
                    ...
                    // Search cell index corrected for periodic boundaries.
                    Index<NDIM> periodic_search_cell_idx;
                    ...
                    for (int k = 0; k < NDIM; ++k)
                    {
                        ...
                        periodic_search_cell_idx[k] = (search_cell_idx[k] -
                                                    cell_offset[k]*max_cell_idx[k]);
                    }
                    // Boolean to test that the cell offset is only nonzero in
                    // periodic directions.  If false, don't apply forces.
                    bool periodic_check = true;
                    for (int k = 0; k < NDIM; ++k)
                    {
                        periodic_check = periodic_check && (cell_offset[k] == 0 || periodic_shift[k] > 0);
                    }
                    if (periodic_check)
                    {
                        LNodeSet* search_node_set =
                            current_idx_data->getItem(periodic_search_cell_idx);
                        if (search_node_set)
                        {
